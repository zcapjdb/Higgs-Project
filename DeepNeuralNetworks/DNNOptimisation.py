...
for hyperparameterOneValue in hyperparameterOne:
    for hyperparameterTwoValue in hyperparameterTwo:
        ...
        for nJets in [2,3]:
            ...
            # Train NN
            modelEven.fit(xEven,yEven,sample_weight = wEven, epochs=epochs, batch_size=batchSize,verbose = 2)
            modelOdd.fit(xOdd,yOdd,sample_weight = wOdd, epochs=epochs, batch_size=batchSize,verbose = 2)
...
for i in graphs:
    ...
    if i == '2 Jets':
        data = np.reshape(dataStorage[:,2], (len(hyperparameterTwo), len(hyperparameterOne)))

    if i == '3 Jets':
        data = np.reshape(dataStorage[:,4], (len(hyperparameterTwo), len(hyperparameterOne)))

    if i == 'Combined':
        data = np.reshape(dataStorage[:,6], (len(hyperparameterTwo), len(hyperparameterOne)))
