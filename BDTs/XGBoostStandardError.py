...
for i in range (0,numberOfIterations):
    ...
    for nJets in [2,3]:
        ...
        if nJets == 2:
            ...
            n_estimators = 176 # 200
            max_depth = 10 # 4
            learning_rate = 0.102375 # 0.15
            subsample = 0.84696 # 0.5
...
for i in graphs:
    ...
    plt.figure()
    n, bins, patches = plt.hist((data), 50, normed=True, alpha=0.7, rwidth=0.75, color='#071BCB')

    # find range for gaussian curve
    xmin, xmax = np.percentile(data, 1), np.percentile(data, 99)
    ...
    figureName = "SUPERNEWXGBoost_500Iterations_" + str(name) + "FIXED.pdf"
    fig = plt.gcf()
